...
def test_grayscale():
	...
	cv2.imwrite("outputs/1/Grayscale.png", grayscale_cv)

	# grayscale_xd = xd.convert_to_grayscale_from_hsi(image)
	grayscale_xd = xd.convert_to_grayscale(image)
	cv2.imwrite("outputs/1/Grayscale_xd.png", grayscale_xd)

def test_gaussian_blur():
	image = cv2.imread('./outputs/1/Grayscale.png')
	blurred_cv = cv2.GaussianBlur(image, (5, 5), 1.4)
	cv2.imwrite("outputs/1/Blurred.png", blurred_cv)

	blurred_xd = xd.gaussian_blur(image, 5, 1.4)
	cv2.imwrite("outputs/1/Blurred_xd.png", blurred_xd)

def test_canny():
	image = cv2.imread('./outputs/10/Grayscale Image.png', cv2.IMREAD_GRAYSCALE)


	edges_cv = cv2.Canny(image, 50, 200, apertureSize=3)
	cv2.imwrite("outputs/1/Edges.png", edges_cv)

	edges_xd = xd.canny_edge_detection(image, 0.05, 0.15, 1.4)
	cv2.imwrite("outputs/1/Edges_xd.png", edges_xd)

def test_dilate():
	image = cv2.imread('./outputs/1/Lines.png')
	kernel = np.ones((5, 5), np.uint8)
	dilated_cv = cv2.dilate(image, kernel, iterations=1)
	cv2.imwrite("outputs/1/Dilated.png", dilated_cv)

	dilated_xd = xd.dilate(image, kernel, 1)
	cv2.imwrite("outputs/1/Dilated_xd.png", dilated_xd)

def test_hough_transform():
	image = cv2.imread('./outputs/1/Edges.png', cv2.IMREAD_GRAYSCALE)
	...
	cv2.imwrite("outputs/1/Hough_Lines.png", hough_lines)
	...
	for line in lines_xd:
		x1, y1, x2, y2 = line
		cv2.line(hough_lines, (x1, y1), (x2, y2), (0, 255, 0), 2)
	cv2.imwrite("outputs/1/Hough_Lines_xd.png", hough_lines)
